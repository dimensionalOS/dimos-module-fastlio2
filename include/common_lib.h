...
// Non-ROS IMU message pointer (mirrors the FAST-LIO2 non-ROS module bridge).
...
// Non-ROS time helpers: custom_messages::Time carries sec/nsec with toSec()/fromSec().
inline double get_time_sec(const custom_messages::Time &time)
{
  return time.toSec();
}

inline custom_messages::Time get_ros_time(double timestamp)
{
  custom_messages::Time t;
  t.fromSec(timestamp);
  return t;
}
